...
LRESULT CGameApp::DisplayWndProc( HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam )
{
	...
	switch (Message)
	{
      ...
      case 'Q':
        ...
        m_pSecondPlayer->Explode();
        m_pSecondPlayer->DecreaseLives();
        break;
			case VK_RETURN:
				fTimer = SetTimer(m_hWnd, 1, 75, NULL);
      ...
      case 'P':
        ...
        break;
      case 'Z':
        m_pSecondPlayer->RotateLeft();
        break;
      case 'X':
        m_pSecondPlayer->RotateRight();
        break;
      case 'C':
        m_pSecondPlayer->Shoot();
        break;
			}
			break;
		...
		case WM_TIMER:
			...
			{
			case 1:
				if(!m_pPlayer->AdvanceExplosion() && !m_pSecondPlayer->IsExploding())
					KillTimer(m_hWnd, 1);
        if (!m_pSecondPlayer->AdvanceExplosion() && !m_pPlayer->IsExploding())
          KillTimer(m_hWnd, 1);
			}
			break;
...
bool CGameApp::BuildObjects()
{
  ...
  m_pSecondPlayer = new CPlayer(m_pBBuffer, mFiredBullets);
...
void CGameApp::SetupGameState()
{
  ...
  m_pSecondPlayer->Position() = Vec2(400, 100);
  m_pSecondPlayer->RotateRight();
  m_pSecondPlayer->RotateRight();
}
...
void CGameApp::ReleaseObjects( )
{
  ...
  if (m_pSecondPlayer != NULL)
  {
    delete m_pSecondPlayer;
    m_pSecondPlayer = NULL;
  }
...
void CGameApp::FrameAdvance()
{
	...
	if ( m_LastFrameRate != m_Timer.GetFrameRate() )
	{
		...
		sprintf_s( TitleBuffer, _T("Game : %s  Lives: %d-%d"), FrameRate, 
               m_pPlayer->GetLives(), m_pSecondPlayer->GetLives() );
  ...
  else if (!m_pSecondPlayer->GetLives())
  {
...
void CGameApp::ProcessInput( )
{
  ...
  if (pKeyBuffer['W'] & 0xF0) DirectionSecond |= CPlayer::DIRECTION::DIR_FORWARD;
  if (pKeyBuffer['S'] & 0xF0) DirectionSecond |= CPlayer::DIRECTION::DIR_BACKWARD;
  if (pKeyBuffer['A'] & 0xF0) DirectionSecond |= CPlayer::DIRECTION::DIR_LEFT;
  if (pKeyBuffer['D'] & 0xF0) DirectionSecond |= CPlayer::DIRECTION::DIR_RIGHT;

	// Move the player
	m_pPlayer->Move(Direction);
  m_pSecondPlayer->Move(DirectionSecond);
  ...
  auto found = find_if(mFiredBullets.begin(), mFiredBullets.end(), [this](const Bullet & aBullet) {
    return m_pPlayer->IsShot(&aBullet);
  });
  ...
  {
    found = find_if(mFiredBullets.begin(), mFiredBullets.end(), [this](const Bullet & aBullet) {
      return m_pSecondPlayer->IsShot(&aBullet);
    });
    if (found != mFiredBullets.end())
    {
      ::SendMessage(m_hWnd, WM_KEYDOWN, 'Q', 0);
      mFiredBullets.erase(found);
    }
  }
...
void CGameApp::AnimateObjects()
{
  ...
  m_pSecondPlayer->Update(m_Timer.GetTimeElapsed(), rectangle);
