...
void CPlayer::Move(ULONG ulDirection,bool stop)
{
	if (stop == 0)
	{
...
bool CPlayer::TestOutOfBounds(HWND window)
{
	RECT rect; 
	...
	if ((m_pSprite->mPosition.y+64)<=0)
	{
		return 0;
	}
	if ((m_pSprite->mPosition.y - 64) >= height)
	{
		return 0;
	}
	if ((m_pSprite->mPosition.x-45) <= 0 )
	{
		return 0;
	}
	if ((m_pSprite->mPosition.x+45) >=width)
	{
		return 0;
	}
	return 1;
