		...
		case WM_TIMER:
			...
			case 1:
				if(!m_pPlayer->AdvanceExplosion())
					KillTimer(m_hWnd, 1);
				if (!m_pPlayerSecondary->AdvanceExplosion())
					KillTimer(m_hWnd, 1);
			}
			break;
...
bool CGameApp::BuildObjects()
{
	...
	m_pPlayerSecondary = new CPlayer(m_pBBuffer);
...
void CGameApp::SetupGameState()
{
	...
	m_pPlayerSecondary->Position() = Vec2(200, 400);
}
...
void CGameApp::ProcessInput()
{
	...
	if (m_pPlayer->TestOutOfBounds(m_hWnd))
	{
		// Move the player
		m_pPlayer->Move(Direction,false);
    }
	else m_pPlayer->Move(Direction, true);
...
void CGameApp::AnimateObjects()
{
	...
	m_pPlayerSecondary->Update(m_Timer.GetTimeElapsed());
}
